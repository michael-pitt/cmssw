...
def _modify2024(process):
    print('Process customised for 2024 PPS era')
    process.load('SimPPS.DirectSimProducer.simPPS2022_cfi')
    if hasattr(process, 'generator'):
        process.generator.energy = process.profile_2022_default.ctppsLHCInfo.beamEnergy
    if hasattr(process, 'ctppsGeometryESModule'):
...
def _modify2025(process):
    print('Process customised for 2025 PPS era')
    process.load('SimPPS.DirectSimProducer.simPPS2022_cfi')
    if hasattr(process, 'generator'):
        process.generator.energy = process.profile_2022_default.ctppsLHCInfo.beamEnergy
    if hasattr(process, 'ctppsGeometryESModule'):
...
modifyConfigurationStandardSequencesFor2024_ = eras.ctpps_2024.makeProcessModifier(_modify2024)
